...
fun AddIncomeScreen(
    onBackClick: () -> Unit,
    onSaveIncome: (description: String, amount: Double, category: IncomeCategory, date: LocalDate, source: ExpenseSource, status: ExpenseStatus, recurrence: RecurrenceRule?, existingIncome: Income?) -> Unit,
    ...
    var description by remember(existingIncome?.id) { mutableStateOf(existingIncome?.description ?: "") }
    var amount by remember(existingIncome?.id) { mutableStateOf(existingIncome?.amount?.toString() ?: "") }
    var selectedCategory by remember(existingIncome?.id) { mutableStateOf(existingIncome?.category ?: IncomeCategory.SALARY) }
    var date by remember(existingIncome?.id) { mutableStateOf(existingIncome?.date ?: today()) }
    var source by remember(existingIncome?.id) { mutableStateOf(existingIncome?.source ?: ExpenseSource.MANUAL) }
    var status by remember(existingIncome?.id) { mutableStateOf(existingIncome?.status ?: ExpenseStatus.CONFIRMED) }
    var recurrenceEnabled by remember(existingIncome?.id) { mutableStateOf(existingIncome?.recurrence != null) }
    var recurrenceFrequency by remember(existingIncome?.id) { mutableStateOf(existingIncome?.recurrence?.frequency ?: RecurrenceFrequency.MONTHLY) }
    var recurrenceInterval by remember(existingIncome?.id) { mutableStateOf(existingIncome?.recurrence?.interval?.toString() ?: "") }
    var recurrenceCount by remember(existingIncome?.id) { mutableStateOf(existingIncome?.recurrence?.count?.toString() ?: "") }
    var recurrenceUntil by remember(existingIncome?.id) { mutableStateOf(existingIncome?.recurrence?.until?.toString() ?: "") }
    var freeHandDateText by remember(existingIncome?.id) { mutableStateOf(date.toString()) }
    var dayText by remember(existingIncome?.id) { mutableStateOf(date.day.toString()) }
    var month by remember(existingIncome?.id) { mutableStateOf(date.month) }
    var year by remember(existingIncome?.id) { mutableStateOf(date.year.toString()) }
    ...
    Scaffold(
        ...
        topBar = {
            TopAppBar(
                ...
                title = {
                    Text(
                        if (existingIncome == null) "Add Income" else "Edit Income",
        ...
        Column(
            ...
            OutlinedTextField(
                ...
                trailingIcon = {
                    TextButton(onClick = {
                        val values = IncomeCategory.values()
                        val currentIndex = values.indexOf(selectedCategory)
                        selectedCategory = values[(currentIndex + 1 + values.size) % values.size]
                    }) { Text("Next") }
                },
                modifier = Modifier.fillMaxWidth(),
                shape = RoundedCornerShape(12.dp),
            ...
            Row(modifier = Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.SpaceBetween) {
                Text("Planned income")
                Switch(
                    checked = status == ExpenseStatus.PLANNED,
                    onCheckedChange = { status = if (it) ExpenseStatus.PLANNED else ExpenseStatus.CONFIRMED }
                )
            }
                ...
                Row(modifier = Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.SpaceBetween) {
                    ...
                    Switch(
                        ...
                        onCheckedChange = {
                            ...
                            if (it && recurrenceInterval.isBlank()) recurrenceInterval = "1"
                        }
                    )
                }
            ...
            Column(verticalArrangement = Arrangement.spacedBy(12.dp)) {
                Text("Source", fontWeight = FontWeight.SemiBold, fontSize = 16.sp)
                OutlinedTextField(
                    value = source.name,
                    onValueChange = {},
                    readOnly = true,
                    ...
                    trailingIcon = {
                        TextButton(onClick = {
                            val values = ExpenseSource.values()
                            val idx = values.indexOf(source)
                            source = values[(idx + 1) % values.size]
                        }) { Text("Next") }
                    },
                    modifier = Modifier.fillMaxWidth(),
                    shape = RoundedCornerShape(12.dp),
            ...
            Button(
                onClick = {
                    ...
                    onSaveIncome(description, amt, selectedCategory, safeDate, source, status, recurrenceRule, existingIncome)
                    onBackClick()
                },
                ...
                Text(
                    if (existingIncome == null) "Save Income" else "Update Income",
...
private fun IncomeCategory.displayName(): String = name.lowercase().replaceFirstChar { it.uppercase() }
